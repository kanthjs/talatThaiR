...
vignette: >
  %\VignetteIndexEntry{Commodity Prices}
...
pork_monthly_2569 |>
  dplyr::arrange(month) |>
  dplyr::select(year_th, month, price)
```
...
q1_cassava <- get_monthly_prices(
  ...
  dplyr::filter(month %in% c("01", "02", "03"))

# คำนวณราคาเฉลี่ยไตรมาส
q1_avg <- q1_cassava |>
  dplyr::group_by(year_th) |>
  dplyr::summarise(
    q1_avg = mean(price, na.rm = TRUE),
    q1_min = min(price, na.rm = TRUE),
    q1_max = max(price, na.rm = TRUE)
  )
print(q1_avg)
```
...
rice_names <- show_daily_products() |>
  dplyr::filter(grepl("^RICE_", Code)) |>
  dplyr::pull(Name)

rice_prices <- lapply(rice_names, function(name) {
  tryCatch(
    get_daily_prices(
      product_code = names(rice_names[which(rice_names == name)]),
      start_date = "2026-01-01",
      end_date = "2026-03-31"
    ) |>
      dplyr::summarise(avg_price = mean(price), .by = product_name),
...
track_prices <- function(product_code, start_date, end_date) {
  get_daily_prices(
    ...
    dplyr::select(product_code, data_date, price, year_th, month)
}

# ติดตามราคามะนาวหลายขนาด พร้อมกัน
products_to_track <- c("LIME_XL", "LIME_L", "LIME_M", "LIME_S")

all_lime_prices <- lapply(products_to_track, track_prices,
  start_date = "2026-01-01",
  end_date = "2026-03-31"
) |>
  do.call(rbind, .)

# วิเคราะห์ค่าเฉลี่ยเฉลี่ยวัน
all_lime_prices |>
  dplyr::group_by(data_date) |>
  dplyr::summarise(
    spread = max(price) - min(price),
    across(starts_with("LIME"), mean, .names = "avg_{.col}")
  )
```

## การแจกแยกตามจังหวัดและจังหวัด

```{r}
# ดึงข้อมูลราคาสุกรมีชีวิต
pork_prices <- get_weekly_prices(product_code = "PORK_LIVE_100")

# แยกตามเดือนและปี
pork_monthly <- pork_prices |>
  dplyr::mutate(
    year_numeric = as.numeric(year_th) - 543, แปลงเป็นค.ศ.
    year_month = paste0(year_th, "-", month)
  ) |>
  dplyr::arrange(year_numeric, month)

# สร้าง pivot table ดูราคาแบบ matrix
pork_pivot <- pork_monthly |>
  tidyr::pivot_wider(
    names_from = month,
    values_from = price,
    values_fn = mean
