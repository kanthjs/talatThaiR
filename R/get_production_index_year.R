#' ดึงข้อมูลดัชนีผลผลิตสินค้าเกษตรรายปี (Yearly Production Index)
#'
...
#' @param date ค้นหาแบบระบุปี (ระบบจะดึงปี พ.ศ. ให้อัตโนมัติ เช่น "2026-10-01" = ดึงข้อมูลของปี 2569)
...
#'
#' @return data.frame ข้อมูลดัชนีผลผลิตรายปี
#' @export
get_production_index_year <- function(
  ...
  .fetch_single_page <- function(p_page, p_date) {
    ...
    # สับราง Endpoint อัตโนมัติ สำหรับรายปี
    if (!is.null(p_date)) {
      path <- "api/production-index-year/all"
      d <- as.Date(p_date)
      
      # แปลงวันที่เป็น ปี พ.ศ. (ไม่ต้องส่งเดือนหรือไตรมาส)
      ...
      # หมายเหตุ: ขออนุญาตใช้ตาราง Mapping ของรายเดือนไปก่อน 
      # ถ้าชื่อสินค้ารายปีต่างออกไป อาจจะต้องสร้างตาราง .YEARLY_PRODUCT_MAP เพิ่มภายหลังครับ
  ...
  while(keep_fetching) {
    ...
    # ดักจับและสร้างคอลัมน์ date จาก year_th
    if (!"date" %in% colnames(temp_data)) {
       if ("data_date" %in% colnames(temp_data)) {
           temp_data$date <- temp_data$data_date
       } else if ("year_th" %in% colnames(temp_data)) {
           calc_year <- as.numeric(temp_data$year_th) - 543
           # กำหนดให้เป็นวันที่ 1 มกราคม ของปีนั้นๆ เพื่อง่ายต่อการเปรียบเทียบ
           temp_data$date <- as.Date(paste0(calc_year, "-01-01"))
       } else {
           stop("\u0e44\u0e21\u0e48\u0e1e\u0e1a\u0e04\u0e2d\u0e25\u0e31\u0e21\u0e19\u0e4c\u0e27\u0e31\u0e19\u0e17\u0e35\u0e48 (date/year_th) \u0e08\u0e32\u0e01 API")
       }
    }
