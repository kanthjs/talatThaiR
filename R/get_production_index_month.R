#' ดึงข้อมูลดัชนีผลผลิตสินค้าเกษตรรายเดือน (Monthly Production Index)
#'
#' @param category_code รหัสหมวดหมู่สินค้า (อ้างอิง show_index_categories() เช่น "LIVESTOCK")
#' @param product_code รหัสสินค้า (อ้างอิง show_index_products() เช่น "BANANA_HOM_THONG")
#' @param group_code รหัสกลุ่มสินค้า (อ้างอิง show_index_groups() เช่น "OIL_CROP")
...
#'
#' @return data.frame ข้อมูลดัชนีผลผลิต
#' @export
get_production_index_month <- function(
  ...
  # อัปเดต: เพิ่มการนับ group_code เข้าไปในระบบด้วย
  ...
  .fetch_single_page <- function(p_page, p_date) {
    ...
    # สับราง Endpoint อัตโนมัติ ครบทั้ง 4 เส้นทาง
    if (!is.null(p_date)) {
      ...
      # อัปเดต: เพิ่มระบบสับรางสำหรับ group_code
