...
#' @param product_code Product code (see `show_quarter_products()`, e.g. "GARLIC_DRY_MIX").
#'   Note: quarterly prices use a separate product map from monthly prices.
#' @param year_th Thai Buddhist year (e.g. 2568). Used with `quarter` for the `/all` endpoint.
#' @param quarter Quarter as integer 1-4 (e.g. 1 or 4). Optional filter for category/group/product;
...
#'   No other parameters should be specified. Default: `FALSE`.
#' @param api_key API key (if required)
#'
#' @return A data.frame of quarterly production index records
#' @export
#'
#' @examples
...
get_production_index_quarter <- function(
  ...
  if (isTRUE(sector)) {
    if (has_cat || has_group || has_prod || has_year || has_quarter) {
      stop("When 'sector = TRUE', no other parameters should be specified.")
    }
    return(.piq_fetch_all("api/production-index-quarter/sector", list(), api_key))
  }
  ...
  if (!has_cat && !has_group && !has_prod) {
    ...
    if (has_year && !has_quarter) {
      stop("'year_th' requires 'quarter' when used in standalone mode (without category_code, group_code, or product_code).")
    }
  }
  ...
  if (has_prod && !(product_code %in% names(.QUARTER_PRODUCT_MAP))) {
    stop(sprintf(
      "Product code '%s' not found. Use show_quarter_products() to see available codes.",
      product_code
    ))
  }
  ...
  if (has_cat) {
    ...
    params <- list(product_name = .QUARTER_PRODUCT_MAP[[product_code]])
    if (has_quarter) params$quarter <- quarter
  ...
  .piq_fetch_all(path, params, api_key)
}
...
# ---------------------------------------------------------------------------
.piq_fetch_all <- function(path, base_params, api_key) {
