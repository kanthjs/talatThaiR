#' ดึงข้อมูลดัชนีราคาสินค้าเกษตรรายไตรมาส (Quarterly Price Index)
#'
...
#'
#' @return data.frame ข้อมูลดัชนีราคารายไตรมาส
#' @export
get_price_index_quarter <- function(
  ...
  .fetch_single_page <- function(p_page, p_date) {
    ...
    # สับราง Endpoint อัตโนมัติ สำหรับรายไตรมาส (price-index-quarter)
    if (!is.null(p_date)) {
      ...
      # ใช้ตารางเฉพาะของรายไตรมาส (.QUARTER_PRODUCT_MAP)
